...
# https://www.kaggle.com/abhmul/leaf-classification/keras-convnet-lb-0-0052-w-visualization
root = 'C:\\Users\\Keston Crandall\\Downloads\\leaf_data'
np.random.seed(2016)
...
def combined_model():
    ...
    # Adhd a fully connected layer just like in a normal MLP
...
my_generator =combined_generator(imgen_train, X_num_tr)
next(my_generator)
...
def construct_y_predictions(list_of_predictions):
    predictions = []
    for i in list_of_predictions:
        highest_pridiction = -1
        index = 0
        index_highest_prediction = None
        for i2 in i:
            if i2 > highest_pridiction:
                highest_pridiction = i2
                index_highest_prediction = index
            index += 1
        predictions.append(index_highest_prediction)
    return predictions
...
# yPred_proba = model.predict([X_img_val, X_num_val])
# con_matrix = confusion_matrix(construct_y_predictions(y_val_cat),construct_y_predictions(yPred_proba))
# pd_cm = pd.DataFrame(con_matrix)
# np.savetxt("val_cm.txt",pd_cm,fmt="%d")

# yPred_proba = model.predict([X_img_tr, X_num_tr])
# con_matrix = confusion_matrix(construct_y_predictions(y_tr_cat),construct_y_predictions(yPred_proba))
# pd_cm = pd.DataFrame(con_matrix)
# np.savetxt("tr_cm.txt",pd_cm,fmt="%d")
